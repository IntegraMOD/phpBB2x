...
$page_title = isset($lang['digest_page_title']) ? $lang['digest_page_title'] : 'Digest';
	...
	else 
	{
		...
		if ($_POST['create_new'] == '1')// new digest
		{
			$sql = 'INSERT INTO ' . DIGEST_SUBSCRIPTIONS_TABLE . ' (user_id, digest_type, format, show_text, show_mine, new_only, send_on_no_messages, send_hour, text_length) VALUES (' .
				...
				"'" . htmlspecialchars((string) $_POST['digest_type']) . "', " .
				"'" . htmlspecialchars((string) $_POST['format']) . "', " .
				"'" . htmlspecialchars((string) $_POST['show_text']) . "', " .
				"'" . htmlspecialchars((string) $_POST['show_mine']) . "', " .
				"'" . htmlspecialchars((string) $_POST['new_only']) . "', " .
				"'" . htmlspecialchars((string) $_POST['send_on_no_messages']) . "', " .
				"'" . intval($send_hour) . "', " .
		...
		else 
		{
			$sql = 'UPDATE ' . DIGEST_SUBSCRIPTIONS_TABLE . ' SET ' .
				"digest_type = '" . htmlspecialchars((string) $_POST['digest_type']) . "', " .
				"format = '" . htmlspecialchars((string) $_POST['format']) . "', " .
				"show_text = '" . htmlspecialchars((string) $_POST['show_text']) . "', " .
				"show_mine = '" . htmlspecialchars((string) $_POST['show_mine']) . "', " .
				"new_only = '" . htmlspecialchars((string) $_POST['new_only']) . "', " .
				"send_on_no_messages = '" . htmlspecialchars((string) $_POST['send_on_no_messages']) . "', " .
		...
		if (!isset($_POST['all_forums']) || $_POST['all_forums'] !== 'on') 
		{
			foreach ($_POST as $key => $value) 
			{
				if (strpos((string) $key, 'forum_') === 0) 
				{
					$sql = 'INSERT INTO ' . DIGEST_SUBSCRIBED_FORUMS_TABLE . ' (user_id, forum_id) VALUES (' .
					$userdata['user_id'] . ', ' . htmlspecialchars(substr((string) $key, 6)) . ')';
					if (!($result = $db->sql_query($sql)))
					 {
