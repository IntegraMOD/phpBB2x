...
class Template {
	...
	var $xs_versiontxt = '2.4.2'; // text version
	...
	function load_config($root, $edit_db)
	{
		...
		for($i=0; $i<(is_countable($outdated) ? count($outdated) : 0); $i++)
		{
		...
		// install/upgrade
		if($edit_db && (((is_countable($add) ? count($add) : 0) > 0) || ((is_countable($up) ? count($up) : 0) > 0) || ((is_countable($del) ? count($del) : 0) > 0)))
		{
			...
			if(!empty($db))
			{
				...
				for($i=0; $i<(is_countable($add) ? count($add) : 0); $i++)
				{
	...
	function subtemplates_make_filename($filename)
	{
		...
		if ( isset($sub_templates) )
		{
			...
			{
				$found = false;
				$num_fids = is_countable($fids) ? count($fids) : 0;
	...
	function compile_code($filename, $code, $use_isset = false)
	{
		...
		$count = is_countable($code_lines) ? count($code_lines) : 0;
		for ($i = 0; $i < ($count - 1); $i++)
		{
			$code_lines[$i] .= ' -->';
		}
		...
		$new_code = array();
		$line_count = is_countable($code_lines) ? count($code_lines) : 0;
	...
	*/
	function _compile_text($code, $use_isset)
	{
		...
		if((is_countable($search) ? count($search) : 0) > 0)
		{
			$code = str_replace($search, $replace, $code);
	...
	function write_cache($filename, $code)
	{
		...
		if(!$file)
		{
			...
			$count = is_countable($dirs) ? count($dirs) : 0;
			if($count > 0)
			for($i=0; $i<$count-1; $i++)
			{
				if($i>0)
	...
	*/
	function _refresh_config($tpl, $add_vars = false)
	{
		...
		if(@file_exists($phpbb_root_path . 'templates/' . $tpl . '/xs_config.cfg'))
		{
			$style_config = array();
			include($phpbb_root_path . 'templates/' . $tpl . '/xs_config.cfg');
			if(is_countable($style_config) ? count($style_config) : 0)
			{
				global $board_config, $db;
				for($i=0; $i<(is_countable($style_config) ? count($style_config) : 0); $i++)
				{
...
{
	return (isset($tpl->_tpldata[$name.'.']) && (is_countable($tpl->_tpldata[$name.'.']) ? count($tpl->_tpldata[$name.'.']) : 0) > 0);
}
