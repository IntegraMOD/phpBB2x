...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	if ( $result = $db->sql_query($sql) )
	{
		if ($row = $db->sql_fetchrow($result)) 
		{
			...
			$hashed_password = password_hash((string) $user_password, PASSWORD_BCRYPT);

			$sql = "UPDATE " . USERS_TABLE . " 
					SET user_newpasswd = '" . $db->sql_escape($hashed_password) . "', user_actkey = '$user_actkey'  
			...
			$emailer->assign_vars(array(
				...
				'USERNAME' => $username,
				'PASSWORD' => $user_password,  // Ensure this is handled securely in templates
