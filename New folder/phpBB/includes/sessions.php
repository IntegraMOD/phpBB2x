...
function session_begin($user_id, $user_ip, $page_id, $auto_create = 0, $enable_autologin = 0, $admin = 0)
{
	...
	if ($user_id != ANONYMOUS)
	{
		if (isset($sessiondata['autologinid']) && (string) $sessiondata['autologinid'] != '' && $user_id) 
		{
			...
			// Verify Bcrypt-hashed auto-login key instead of MD5
			if ($userdata && password_verify($sessiondata['autologinid'], $userdata['key_id'])) 
			{
	...
	{
		$session_id = bin2hex(random_bytes(16)); // Generate a secure session ID
		$priv_session_id = password_hash(dss_rand(), PASSWORD_BCRYPT); // Use Bcrypt instead of MD5
	...
	if ($user_id != ANONYMOUS) 
	{
		...
		// Regenerate the auto-login key securely
		if ($enable_autologin) 
		{
			$auto_login_key = dss_rand() . dss_rand();
			$hashed_auto_login_key = password_hash($auto_login_key, PASSWORD_BCRYPT);

			if (isset($sessiondata['autologinid']) && (string) $sessiondata['autologinid'] != '') 
			{
				$sql = "UPDATE " . SESSIONS_KEYS_TABLE . "
					SET last_ip = '$user_ip', key_id = '$hashed_auto_login_key', last_login = $current_time
					WHERE user_id = $user_id";
			} 
			else 
			{
				$sql = "INSERT INTO " . SESSIONS_KEYS_TABLE . "(key_id, user_id, last_ip, last_login)
					VALUES ('$hashed_auto_login_key', $user_id, '$user_ip', $current_time)";
			}
    ...
    $priv_session_id = isset($priv_session_id) ? $priv_session_id : null; // Initialize the variable
...
function session_end($session_id, $user_id)
{
	...
	if (isset($userdata['session_key']) && $userdata['session_key'] != '') 
	{
		$sql = "DELETE FROM " . SESSIONS_KEYS_TABLE . "
			WHERE user_id = '$user_id'
				AND key_id = '" . $userdata['session_key'] . "'";
...
function session_reset_keys($user_id, $user_ip)
{
    ...
    $key_sql = ($user_id == $userdata['user_id'] && !empty($userdata['session_key'])) ? "AND key_id != '" . $userdata['session_key'] . "'" : '';
    ...
    {
        $auto_login_key = dss_rand() . dss_rand();
        $hashed_key = password_hash($auto_login_key, PASSWORD_BCRYPT);
        $current_time = time();

        $sql = 'UPDATE ' . SESSIONS_KEYS_TABLE . "
            SET last_ip = '$user_ip', key_id = '$hashed_key', last_login = $current_time
            WHERE user_id = $user_id";
        ...
        // Securely rebuild the session cookie
        $sessiondata['userid'] = $user_id;
        $sessiondata['autologinid'] = $auto_login_key;
        $cookiename = $board_config['cookie_name'];
        $cookiepath = $board_config['cookie_path'];
